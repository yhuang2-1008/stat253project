...
# data cleaning
# We had to clean up the csv in a separate non-GitHub related rmd because file was too big
# This is just a record of the code that we used to clean up the large data set

rideshare %>%
select(-id, -timestamp, -datetime, -timezone, -product_id, -surge_multiplier, -apparentTemperature, -long_summary,  -precipIntensity, -windGustTime, -temperatureHigh, -temperatureHighTime, -temperatureLow, -temperatureLowTime, -apparentTemperatureHigh, -apparentTemperatureHighTime, -apparentTemperatureLow, -apparentTemperatureLowTime, -icon, -dewPoint, -pressure, -windBearing, -cloudCover, -uvIndex, -visibility.1, -ozone, -sunriseTime, -sunsetTime, -moonPhase, -precipIntensityMax, -uvIndexTime, -temperatureMin, -temperatureMinTime, -temperatureMax, -temperatureMaxTime, -apparentTemperatureMin, -apparentTemperatureMax, -apparentTemperatureMinTime, -apparentTemperatureMaxTime)

```

```{r}
# creation of cv folds
data_cv29 <- vfold_cv(rideshare, v =29)
```
...
```{r}
# recipes & workflows
rideshare_recipe <- recipe(price ~ . , data = rideshare) %>%
...
model_wf <- workflow() %>%
    add_formula(price ~ cab_type + distance) %>%
    add_model(lm_spec)

lasso_wf_tune <- workflow() %>%
    add_recipe(rideshare_recipe) %>%
