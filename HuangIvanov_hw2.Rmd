...
rideshare <- read.csv("Energy

```

```{r}
# creation of cv folds
cv10 <- vfold_cv(rideshare, v = 10)
```
...
```{r}
# recipes & workflows
ride_recipe <- recipe(price ~ hour+day+month+distance+temperature, data = rideshare) %>%
  update_role(all_nominal(), new_role = "ID") %>%
  update_role(price, new_role = "outcome") %>%
  update_role(hour,day,month,distance,temperature, new_role = "predictor") %>%
...
tmp <- fit(lasso_wf_tune, data = rideshare)
tmp %>% tidy()
```
